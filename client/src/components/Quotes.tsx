export const Quotes = [
  { person: "Khalifatul Masih III", quote: "Love For All, Hatred For None." },
  { person: "Amy Poehler", quote: "Change the world by being yourself." },
  { person: "T.S Eliot", quote: "Every moment is a fresh beginning." },
  { person: "Mark Twain", quote: "Never regret anything that made you smile." },
  { person: "Unknown", quote: "Die with memories, not dreams." },
  { person: "Unknown", quote: "Aspire to inspire before we expire." },
  { person: "Pablo Picasso", quote: "Everything you can imagine is real." },
  {
    person: "Leonardo da Vinci",
    quote: "Simplicity is the ultimate sophistication."
  },
  { person: "Walt Disney", quote: "Whatever you do, do it well." },
  { person: "Buddha", quote: "What we think, we become." },
  {
    person: "Oliver Wendell Holmes",
    quote: "All limitations are self-imposed."
  },
  {
    person: "Robert H. Schiuller",
    quote: "Tough times never last but tough people do."
  },
  {
    person: "Robert H. Schiuller",
    quote: "Problems are not stop signs, they are guidelines."
  },
  {
    person: "Johnny Depp",
    quote:
      "One day the people that don’t even believe in you will tell everyone how they met you."
  },
  {
    person: "Kendrick Lamar",
    quote: "If I’m gonna tell a real story, I’m gonna start with my name."
  },
  {
    person: "Mark Twain",
    quote: "If you tell the truth you don’t have to remember anything."
  },
  {
    person: "Jessica N. S. Yourko",
    quote: "Have enough courage to start and enough heart to finish."
  },
  {
    person: "Tyga",
    quote: "Hate comes from intimidation, love comes from appreciation."
  },
  {
    person: "Harvey Specter",
    quote: "I could agree with you but then we’d both be wrong."
  },
  {
    person: "Dr. Seuss",
    quote: "Oh, the things you can find, if you don’t stay behind."
  },
  {
    person: "Eileen McDargh",
    quote: "Determine your priorities and focus on them."
  },
  { person: "Steve Martin", quote: "Be so good they can’t ignore you." },
  {
    person: "James Dean",
    quote: "Dream as if you’ll live forever, live as if you’ll die today."
  },
  { person: "Nike", quote: "Yesterday you said tomorrow. Just do it." },
  {
    person: "Lil Wayne",
    quote: "I don’t need it to be easy, I need it to be worth it."
  },
  {
    person: "Drake",
    quote: "Never let your emotions overpower your intelligence."
  },
  {
    person: "J. Cole",
    quote: "Nothing lasts forever but at least we got these memories."
  },
  {
    person: "Kendrick Lamar",
    quote: "Don’t you know your imperfections is a blessing?"
  },
  { person: "Tupac", quote: "Reality is wrong, dreams are for real." },
  { person: "Peter Pan", quote: "To live will be an awfully big adventure." },
  { person: "Maya Angelou", quote: "Try to be a rainbow in someone’s cloud." },
  { person: "Thomas Edison", quote: "There is no substitute for hard work." },
  { person: "Unknown", quote: "What consumes your mind controls your life" },
  { person: "Lebron James", quote: "Strive for greatness." },
  {
    person: "Kurt Cobain",
    quote: "Wanting to be someone else is a waste of who you are."
  },
  { person: "Maya Angelou", quote: "And still, I rise." },
  {
    person: "Martin Luther King Jr.",
    quote: "The time is always right to do what is right."
  },
  { person: "Rumi", quote: "Let the beauty of what you love be what you do." },
  {
    person: "Nelson Mandela",
    quote: "May your choices reflect your hopes, not your fears."
  },
  {
    person: "Atticus",
    quote: "A happy soul is the best shield for a cruel world."
  },
  {
    person: "Habeeb Akande",
    quote: "White is not always light and black is not always dark."
  },
  {
    person: "R. Brault",
    quote:
      "Life becomes easier when you learn to accept the apology you never got."
  },
  { person: "Aristotle", quote: "Happiness depends upon ourselves." },
  { person: "Oprah Winfrey", quote: "Turn your wounds into wisdom." },
  {
    person: "Macklemore",
    quote: "Change the game, don’t let the game change you."
  },
  { person: "John Green", quote: "It hurt because it mattered." },
  {
    person: "Boonaa Mohammed",
    quote: "If the world was blind how many people would you impress?"
  },
  {
    person: "Unknown",
    quote: "I will remember and recover, not forgive and forget."
  },
  {
    person: "Ken Hudgins",
    quote: "The meaning of life is to give life meaning."
  },
  {
    person: "Nelson Henderson",
    quote:
      "The true meaning of life is to plant trees, under whose shade you do not expect to sit."
  },
  { person: "Shakespeare", quote: "When words fail, music speaks." },
  {
    person: "Elizabeth Gilbert",
    quote: "Embrace the glorious mess that you are."
  },
  {
    person: "Vincent van Gogh",
    quote:
      "Normality is a paved road; it’s comfortable to walk but no flowers grow."
  },
  { person: "Eminem", quote: "I have nothing to lose but something to gain." }
];
